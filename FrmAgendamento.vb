Public Class FrmAgendamento
    ...
    Private Sub Agendamento_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If agenda Then
            TxtData.Text = d
            TxtHora.Text = aux
            sql = "SELECT * FROM TBFUNCIONARIOS WHERE NOME = '" & FrmServicos.CmbProfissional.Text & "'"
            rs = db.Execute(sql)
            CmbTipoServico.Text = rs.Fields(4).Value
            CmbFuncionario.Text = FrmServicos.CmbProfissional.Text
            CmbServico.Text = FrmServicos.CmbServico.Text
